...
import com.gc.service.CustomerAccountService;

@RestController
@RequestMapping(value = "/mortgage-customer-account-service/v1/customer-account")
public class CustomerAccountController extends AbstractRestController {
	...
	public Optional<Account> getSimpleCustomerAccount(@PathVariable("id") Long id) {
    	Optional<Account> account = this.customerAccountService.getAccount(id);
    ...
    public
    @ResponseBody
    Optional<Account> getAccount(@PathVariable("id") Long id,
                             HttpServletRequest request, HttpServletResponse response) throws Exception {
    	Optional<Account> account = this.customerAccountService.getAccount(id);
        checkResourceFound(account);
        return account;
